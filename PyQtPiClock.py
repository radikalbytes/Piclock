...
sonidoAlarma1 = pygame.mixer.Sound('SD_ALERT_44.wav')
sonidoAlarma2 = pygame.mixer.Sound('SD_ALERT_43.wav')
def tick2():
    global posicionx
    """
    for i in range(0, 9):
        forecast[i].setGeometry((((i-3) * 220) + posicionx) * xscale, 650 * yscale,
                    220 * xscale, 100 * yscale)
...
def tick():
    ...
    global clockrect
    global datex, datex2, textoAlarma, datey2, pdy
    global sonidoAlarma1, sonidoAlarma2
    global alarmas
    ...
    if now.second == 0:
        textoAlarma.setText("")
        for qa in range (len(alarmas)):
            if qa%2==0:
                if now.hour==alarmas[qa].hour:
                    if now.minute==alarmas[qa].minute:
                        textoAlarma.setText(alarmas[qa+1])
                        sonidoAlarma1.play()
    ...
    if now.day != lastday:
        ...
        # date
        sup = ""
        #ds = "{0:%A %B} {0.day} {0.year}".format(now, sup)
        ds = "{0.day} {0: %B} {0.year}".format(now, sup)
        datex.setText(ds)
        datex2.setText(ds)
...
def qtstart():
    global ctimer, ctimer2, wxtimer, temptimer
    global manager
    global objradar1
    global objradar2
    global objradar3
    ...
    ctimer2 = QtCore.QTimer()
    ctimer2.timeout.connect(tick2)
    ctimer2.start(1)
...
def myquit(a=0, b=0):
    ...
    global ctimer, ctimer2, wtimer, temptimer
...
textoAlarma = QtGui.QLabel(frame1)
textoAlarma.setObjectName("textoAlarma")
textoAlarma.setStyleSheet("#textoAlarma { font-family:sans-serif; color: " +
                    Config.textcolor +
                    "; background-color: transparent; font-size: " +
                    str(int(80 * xscale)) +
                    "px; " +
                    Config.fontattr +
                    "}")
textoAlarma.setAlignment(Qt.AlignHCenter | Qt.AlignTop)
textoAlarma.setGeometry(0, 50 * yscale, width, 100)
...
datex.setGeometry(0, 540 * yscale, width, 100)
...
for i in range(0, 9):
    ...
    lab.setGeometry((((i-3) * 220)+50 ) * xscale, 650 * yscale,
                    220 * xscale, 100 * yscale)
...
archivo = open("Alarmas.txt","r")
indice = 0
alarmas = []
while True:
    _hora=archivo.readline()
    if not _hora:
        break
    _texto=archivo.readline()
    if not _texto:
        break
    __hora = str(_hora)
    __hora = __hora[:-1]
    ...
    alarmas.append(datetime.datetime.strptime(__hora,'%H:%M'))
    alarmas.append(_texto)
archivo.close
for f in range (0,len(alarmas)):
    print (alarmas[f])
